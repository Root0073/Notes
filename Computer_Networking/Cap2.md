...
# Stop and Wait
停止等待协议适用于流量控制，解决的基本问题是当发送方发送数据的速度高于接收方

通常的做法是接收方会向发送方提供某种反馈

用一个单比特计数器来解决重复的问题

# 滑动窗口
这也是一个用于流量控制的算法，等待停止协议可能有一个问题就是他的效率很低，因为他一次只允许一个数据报传输

而滑动窗口可以一次允许n个传输，在滑动窗口协议中，每一个段都有他自己的序列号

发送方维护三个变量：发送窗口的大小，从接收方收到的最后一个确认以及他发送到最后一个段

发送方的任务是维护这个不变量即最后一个段减去最后一个收到的必须小于等于发送窗口的大小

窗口无法移动到第一个未被确认的段之后，如果有包丢失在窗口之前那么这个窗口就停滞了

接收方也维护三个变量：接收窗口大小，最后一个可接受的段以及他实际接收到的最后一个段

如果超出了这个范围则会直接丢弃

TCP不是确认他收到的数据而是确认n+1 

在回退-n协议的情况下，你需要发送窗口大小+1的序列号

滑动窗口协议使用累计确认

当他看到窗口内的某个数据丢失时，会重传窗口内的这个数据

# TCP Header
TCP前两个字段是源端口和目标端口，每个字段都是16位

第二个字段是TCP序列号，表示从源段到目标端，该数据段中数据的序列号

该端点返回的确认号

序列号表示紧随段头之后数据区域中第一个字节的序列号

校验和是基于TCP伪头部计算的，覆盖了这个伪头部，TCP头部以及TCP段中的数据
